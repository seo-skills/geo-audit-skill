...
| 27 | **An archive is not an article.** | Presume every page that declares nothing is an article. | A page at `.../category/<name>`, `.../tag/<name>` or `.../author/<name>` (plurals too, optionally paged) is not an article, unless it declares an article type. | Archives list posts that sit beside them, not beneath them, so the index rule misses them: on seomator.com two findings named seven category archives and an about page, and no article. |
